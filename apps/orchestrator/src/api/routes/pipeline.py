...
from src.pipeline.engine import PipelineEngine, StepContext
...
def run_single_step(
    ...
    if page.migration_status not in ("queued", "running", "blocked"):
        if page.migration_status == "complete":
            return {"success": False, "error": {"code": "PIPELINE-002", "message": "Page already complete"}}
    ...
    task_id = f"step-{request.page_id}-{page.current_step + 1}"
    _running_tasks[task_id] = {
        "page_id": request.page_id,
        "step": page.current_step + 1,
        "status": "queued",
        "started_at": datetime.utcnow().isoformat(),
    }
    ...
    return {
        ...
        "data": {
            "task_id": task_id,
            "page_id": request.page_id,
            "next_step": page.current_step + 1,
            "message": f"Running step {page.current_step + 1} for {request.page_id}",
        },
    }
...
def get_page_steps(page_id: str, db: Session = Depends(get_db)):
    ...
    executions = (
        ...
        .order_by(StepExecution.step_number, StepExecution.attempt_number)
        .all()
    )
    ...
    step_names = {
        1: "spec_load", 2: "spec_verify", 3: "api_contract",
        4: "react_generation", 5: "java_generation", 6: "java_test",
        7: "integration_test", 8: "equivalence_check", 9: "complete",
    }
    ...
    for step_num in range(1, 10):
        ...
        steps_data.append({
            ...
            "artifacts": [
                {"type": a.artifact_type, "path": a.file_path, "version": a.version}
