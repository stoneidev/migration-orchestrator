...
- **Phase 3 완료** (2026-05-25) — Alembic 도입(`alembic/`, `alembic.ini`, baseline `cb0a1fb6f34a_baseline_schema.py`). `configure_db`가 on-disk DB는 alembic으로, in-memory 테스트는 `create_all`로 분리 처리, 기존 dev DB는 자동 stamp. FastAPI `lifespan` 사용으로 `on_event` 폐기 경고 제거, EventBus loop 캡처/shutdown 위치 명확. `EventBus`를 `asyncio.Lock` + dead connection 자동 정리 + `shutdown()`까지 완전 재작성. `src/util/clock.py` 추가 후 모든 `datetime.utcnow()` → `utcnow_naive()` 전환. CORS는 wildcard 대신 loopback regex (`^https?://(localhost|127.0.0.1)(:\d+)?$`)로 좁힘. 105 tests passing.
- **Phase 4 완료** (2026-05-25) — `step4_react_gen` 비주얼 비교를 raw pixel diff → SSIM + pixel diff dual-gate로 교체 (single-window global SSIM). `compare_screenshots`가 `VisualComparison(diff_percent, ssim, width, height)` 반환. dev 서버 시작은 고정 `sleep(10)` → TCP 포트 폴링 (`_wait_for_dev_server`). 모든 임계값/포트/대기시간은 `Settings.visual_ssim_threshold`, `visual_diff_threshold_pct`, `frontend_dev_port=3100`, `frontend_dev_startup_timeout_s=30`로 외부화 (3001 하드코딩 제거). `Settings(extra="ignore")`로 구버전 `.env` 호환. 111 tests passing.
- **Phase 5 완료** (2026-05-25) — `routes/spec_gen` 내부 Playwright 인라인 로직(70+ 라인)을 `PlaywrightWorker.capture_for_spec()`으로 추출, `PageCapture` 데이터클래스 신설. `src/logging_setup.py` 추가하여 lifespan 시작 시 단일 root logger 구성 (`ORCH_LOG_LEVEL` 환경변수 지원, botocore/urllib3/websockets는 WARNING으로 축소). 114 tests passing.
- **Phase 6 스펙** (2026-05-25) — 후속 개선 계획은 `docs/phase6-spec.md` 참조 (인증·보안 제외, 6A~6I).
